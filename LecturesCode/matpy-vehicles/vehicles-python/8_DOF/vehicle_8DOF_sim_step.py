...
for tt in range(1,Tsim):
    ...
    ts = np.arange((tt-1)*0.001, tt*0.001, 0.0001)
    delta=delta3[i]
...
#    [t,wz]= ode45(@(t,wz) wz_dot,ts,wz)
    [t,psi]= solve_ivp(@(t,psi) dpsi,ts,psi)
    [t,phi]= solve_ivp(@(t,phi) dphi,ts,phi)
    [t,u]= solve_ivp(@(t,u) u_dot,ts,u)
    [t,v]= solve_ivp(@(t,v) v_dot,ts,v)
    [t,wx]= solve_ivp(@(t,wx) wx_dot,ts,wx)
    [t,wz]= solve_ivp(@(t,wz) wz_dot,ts,wz)
...
#    [t,wrr]=ode45(@(t,wrr) dwrr,ts,wrr)
    [t,wlf]=solve_ivp(@(t,wlf) dwlf,ts,wlf)
    [t,wrf]=sovle_ivp(@(t,wrf) dwrf,ts,wrf)
    [t,wlr]=solve_ivp(@(t,wlr) dwlr,ts,wlr)
    [t,wrr]=solve_ivp(@(t,wrr) dwrr,ts,wrr)
    wlf=wlf[11]
    wrf=wrf[11]
    wlr=wlr[11]
    wrr=wrr[11]
