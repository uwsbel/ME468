...
## 8 DOF vehicle dynamics model: ramp steering anlge input 3(deg/s) and Vx=50/3.6(m/s)
...
#ramp steer steering angle input 3(deg/s) and Vx=50/3.6
delta4 = np.zeros(4700)
for i in range(1, 4700):
    if i < 1000:
        delta4[i]=0
    else: 
        delta4[i]=3*pi/180*(0.001*i-1)

## vehicle initial state
u=50/3.6 # the longitudinal velocity 
...
for tt in range(1, Tsim):
    ...
    ay[i]=u*wz+v_dot
    # record longitudinal slip ratio
    s_re[i,1]=s_lf
    s_re[i,2]=s_rf
    s_re[i,3]=s_lr
    s_re[i,4]=s_rr
...
t = range(0,Tsim-1)
mpl.figure(6)
mpl.plot(t*0.001,delta4*180/pi,'r')
